...
function Chars() {
    ...
    const handleDelete = (id)=>{
        ...
        // axios.delete(`http://localhost:3000/Chars/${id}`)
        .then(()=>{
        ...
        {data.map((item,index)=>(
          <div key={index} className='w-80 rounded-lg text-center flex flex-col items-center justify-center'>
            ...
            <Link to={`/${item.id}`}>
              ...
              px-4 py-2 text-center transition-transform transform-gpu hover:scale-105 
              h-60 bg-cover w-60'></div></Link>
